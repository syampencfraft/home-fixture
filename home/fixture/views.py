...
from .models import Category, ServiceProfessional, Booking, Service, JobTracking, UserProfile, Payment, Review, Notification
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Notification.objects.create(
                ...
                message=f"New booking request for {booking.service.name} from {request.user.username} on {booking.booking_date.date()}."
            )
...
def update_booking_status(request, booking_id, status):
    ...
    if status_upper in dict(Booking.STATUS_CHOICES):
        ...
        Notification.objects.create(
            ...
            message=f"Your booking for {booking.service.name} has been {status_upper.lower()} by {booking.professional.user.username}."
        )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Notification.objects.create(
                ...
                message=f"You received a {review.rating}-star review from {booking.customer.username} for your service."
            )
...
def submit_complaint(request, booking_id):
    ...
    if request.method == 'POST':
        ...
        if description:
            ...
            # Notify professional (optional, but good for transparency)
            Notification.objects.create(
                user=booking.professional.user,
                message=f"A complaint has been filed by {booking.customer.username} regarding booking #{booking.id}."
            )
...
def update_job(request, booking_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Notification.objects.create(
                user=booking.customer,
                message=f"Job details updated for your {booking.service.name} booking by {booking.professional.user.username}."
            )
